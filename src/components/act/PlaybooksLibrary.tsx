...
const PlaybooksLibrary: React.FC<PlaybooksLibraryProps> = ({ 
  ...
  return (
    <div>
      <div
        className="flex items-center justify-center p-2 cursor-pointer hover:bg-white/5 rounded-t-lg transition-colors"
        onClick={onToggleExpanded}
      >
        <ChevronDown 
          className={`h-5 w-5 transition-transform ${expanded ? 'rotate-180' : ''}`} 
        />
        <span className="ml-2">{t('playbooksLibrary', { defaultValue: 'Playbooks Library' })}</span>
      </div>
              ...
              <div className="p-4">
                <div className="flex items-center bg-white/5 border border-white/20 rounded-lg px-3 py-2 mb-4">
                  ...
                  <input
                    ...
                    placeholder={t('searchPlaybooks', { defaultValue: 'Search playbooks...' }) as string}
                    value={searchQuery}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {playbook.tags.map((tag) => (
                              <span 
                                key={`${playbook.id}-${tag}`}
                                className="bg-white/10 text-xs px-2 py-0.5 rounded-full"
                              >
                                {tag}
                              </span>
                            ))}
                  ...
                  <motion.div
                    className="bg-white/5 border border-dashed border-white/20 rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <FilePlus className="h-8 w-8 text-gray-400 mb-2" />
                    <span>{t('addNewPlaybook', { defaultValue: 'Add New Playbook' })}</span>
                  </motion.div>
                </div>
                
                <div className="mt-4 text-center text-sm text-gray-400">
                  {t('dragPlaybookHint', { defaultValue: 'Drag a playbook to the Bundles Rail to create a new bundle' })}
                </div>
              </div>
            </GlassCard>
          </motion.div>
