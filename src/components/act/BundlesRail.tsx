...
const BundlesRail: React.FC<BundlesRailProps> = ({
  ...
  const {
    ...
    isLoading,
    error
  } = useRealBundles(statusFilter === 'all' ? undefined : statusFilter);
  const {
    createBundle,
    updateBundle,
    approveBundle
  } = useRealBundleActions();

  // Convert database bundles to UI bundles
  const filteredBundles = bundles.filter(bundle => bundle.name.toLowerCase().includes(searchTerm.toLowerCase()) || bundle.summary?.toLowerCase().includes(searchTerm.toLowerCase()) || bundle.tags?.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())));
    ...
    if (editingBundle) {
      updateBundle.mutateAsync({
        ...
        const transformedBundle: Bundle = {
          ...
          leveragePoints: Array.isArray(data.leverage_points) ? data.leverage_points.filter((point): point is string => typeof point === 'string') : [],
  ...
  if (error) {
    return <div className="w-80 glass-morphism border-r border-white/10 p-6">
  ...
  return <>
      <div className="flex flex-col h-full w-full">
        ...
        <div className="p-4 border-b border-white/10 space-y-4 flex-shrink-0">
          ...
          <div className="relative">
            ...
            <Input placeholder={t('searchBundles')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>

          {/* Status Filter */}
          <div className="flex flex-wrap gap-1">
            {['all', 'draft', 'active', 'pilot', 'completed'].map(status => <Button key={status} variant={statusFilter === status ? 'default' : 'outline'} size="sm" onClick={() => setStatusFilter(status as any)} className="text-xs px-2 py-1 h-auto">
                {t(status)}
              </Button>)}
          </div>
        </div>
          ...
          <ScrollArea className="h-full">
            <div className="p-3 space-y-3">
              {isLoading ? <div className="space-y-3">
                  {[...Array(5)].map((_, i) => <div key={i} className="h-24 bg-white/5 rounded-lg animate-pulse" />)}
                </div> : filteredBundles.length === 0 ? <div className="text-center py-8 text-gray-400">
                  <Archive className="h-8 w-8 mx-auto mb-2" />
                  <p>{searchTerm ? t('noBundlesFound') : t('noBundlesYet')}</p>
                  {!error && <p className="text-xs mt-1">Create your first bundle to get started</p>}
                </div> : <AnimatePresence>
                  {filteredBundles.map(bundle => <motion.div key={bundle.id} layout initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                opacity: 0,
                y: -20
              }} className={`p-3 rounded-lg glass-panel border cursor-pointer transition-all duration-200 ${selectedBundle?.id === bundle.id ? 'border-teal-500/50 bg-teal-500/10' : 'border-white/20 hover:border-white/30 hover:bg-white/5'}`} onClick={() => handleBundleClick(bundle)}>
                    ...
                    handleEditBundle(bundle);
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          ...
                          {bundle.coherence > 0 && <span className="text-xs text-gray-400">
                              {bundle.coherence}%
                            </span>}
                        </div>
                        {bundle.status === 'draft' && <Button size="sm" variant="ghost" onClick={e => {
                    e.stopPropagation();
                    handleApproveBundle(bundle.id);
                      ...
                      {bundle.tags && bundle.tags.length > 0 && <div className="flex flex-wrap gap-1">
                          {bundle.tags.slice(0, 2).map((tag, index) => <Badge key={index} variant="outline" className="text-xs px-1 py-0 border-gray-600 text-gray-300">
                              {tag}
                            </Badge>)}
                          {bundle.tags.length > 2 && <Badge variant="outline" className="text-xs px-1 py-0 border-gray-600 text-gray-300">
      ...
      {/* Bundle Modal */}
      <BundleModal open={isModalOpen} onOpenChange={setIsModalOpen} initialBundle={editingBundle} onSave={handleSaveBundle} />
    </>;
};
