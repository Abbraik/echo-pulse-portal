    ...
    >
      ...
      <div className="flex items-center justify-between h-full">
        ...
        <div className="flex items-center space-x-4">
          <div className={`p-3 rounded-xl ${config.bg} ${config.pulse}`}>
            <IconComponent size={24} className={config.color} />
          </div>
          
          <div className="space-y-1">
            <h2 className="text-lg font-semibold text-white">Today's Critical Insight</h2>
            <div className="flex items-center space-x-2">
              ...
              <Badge className={`${config.color} ${config.bg} font-bold text-lg px-3 py-1`}>
                {displayData.metric}
              </Badge>
            </div>
            <p className="text-sm text-gray-400">{displayData.change}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          {displayData.hasStrategicAlert && (
            ...
            >
              <Button
                size="lg"
                className={`${config.pulse} ${config.color === 'text-red-400' ? 'bg-red-600 hover:bg-red-700' : 'bg-teal-600 hover:bg-teal-700'} text-white font-semibold px-6 py-3`}
              >
