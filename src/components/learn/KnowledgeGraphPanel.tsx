...
import { Search, ZoomIn, ZoomOut, Filter, ArrowRight } from 'lucide-react';
...
export const KnowledgeGraphPanel: React.FC = () => {
  ...
  const handleZoomIn = () => {
    if (graphRef.current) {
      // Handle zoom in - will implement in the graph component
    }
  };
  
  const handleZoomOut = () => {
    if (graphRef.current) {
      // Handle zoom out - will implement in the graph component
    }
  };
  ...
  return (
    <GlassCard className="overflow-hidden h-full">
      ...
      <GlassCardContent className="h-[calc(100%-84px)]">
