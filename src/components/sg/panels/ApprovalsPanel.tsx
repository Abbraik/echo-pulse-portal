...
const ApprovalsPanel: React.FC<ApprovalsPanelProps> = ({ data, actions }) => {
  ...
  const renderApprovalItem = useMemo(() => (approval: Approval, index: number) => (
    <div className="glass-panel-cinematic p-4 space-y-3 hover:neon-border transition-all duration-300 group mx-2 mb-2">
  ...
  return (
    <div className="space-y-4">
      {data.map((approval, index) => (
        <div key={approval.id}>
          {renderApprovalItem(approval, index)}
        </div>
      ))}
    </div>
  );
...
export default React.memo(ApprovalsPanel);
